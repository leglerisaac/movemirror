...
MoveMirror turns a Chess.com username and a recent-game sample into a practical training report. It replays each public PGN in the browser, summarizes the player's strongest and weakest signals, and ranks three puzzle categories to practice next.

## What it reports

- Win/draw/loss score, average rating, opponent rating, and available Chess.com accuracy
...
MoveMirror uses Chess.com's read-only [Published-Data API](https://www.chess.com/news/view/published-data-api). Requests are made serially to respect the API's rate-limit guidance. The app only analyzes standard chess and can filter for rapid, blitz, bullet, or daily games.

This is a pattern analyzer, not a replacement for engine review. It does not run Stockfish or claim that a move is definitively a blunder. Instead, it detects explainable board signals such as persistent material drops, an undefended piece captured on the next move, double attacks against valuable targets, and vulnerable piece line-ups. Existing Chess.com accuracy is displayed when the API provides it.
...
The smoke test downloads a small current public sample and verifies that the analyzer produces a complete report.
...
- Chess.com PubAPI for public profiles and game archives

MoveMirror is an independent project and is not affiliated with or endorsed by Chess.com.
